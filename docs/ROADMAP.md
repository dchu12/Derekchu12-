...
_Shipped since (v134–v136): fixed "safe to spend today" overstating (now total left ÷ days); **removed the Treat Fund** (v136) — Kelly felt she'd never hit it, so the whole under-budget→treat-money feature (dashboard card, modal, Settings toggle, earn-on-close) is gone; migration drops orphaned state; **auto-trim** — overspending a category proportionally shrinks the "left" shown on the still-under categories so they reconcile with the real left-to-spend (with a plain-language note), fixing the "Food says $114 but top says $65" confusion; hardened category math against string budgets._
...
_Shipped earlier (v126–v130): Light/Dark/Auto theme toggle in Settings; optional app lock (PIN + biometric/WebAuthn, auto-relock, device-local); Home-screen "Quick add" shortcut + share-target (`?action=add` / shared text jumps straight into logging spend); 8-character household invite codes; celebrations — confetti + haptic buzz on logging a spend, fireworks + confetti + buzz on closing a pay period (canvas-based, no libraries, respects prefers-reduced-motion)._

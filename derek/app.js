...
(function () {
  ...
  function migrateState(s) {
    ...
    delete s.treat; // Treat Fund feature removed — drop any orphaned state.
    return s;
  }
  ...
  function renderDashboard(p) {
    ...
    const renderCat = (c) => {
      const cs = catSpent(p, c.id);
      const bud = Number(c.budgeted) || 0; // coerce (budgeted can arrive as a string)
  ...
  if (typeof window !== "undefined") {
    try {
      window.__yosanTest = {
        ...
        safeToSpendPool, safeToSpendToday, discTrim,
        setState: (s) => { state = s; },
        getState: () => state,
      };
